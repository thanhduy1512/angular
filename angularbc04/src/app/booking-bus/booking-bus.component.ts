...
export class BookingBusComponent implements OnInit {
  ...
  onSelect(val: Seat) {
    const foundIndex = this.bookingList.findIndex(item => item.SoGhe === val.SoGhe);
    if (foundIndex === -1) {
      this.bookingList.push(val);
    } else {
      this.bookingList.splice(foundIndex, 1);
    }
  }
  onCancel(soGhe: number) {
    this.bookingList = this.bookingList.filter((item) => item.SoGhe !== soGhe);
    this.lsSeatItem.forEach(seatItemCom => {
      if (seatItemCom.seat.SoGhe === soGhe) {
        seatItemCom.isBooking = false;
      }
    })
  }
}
